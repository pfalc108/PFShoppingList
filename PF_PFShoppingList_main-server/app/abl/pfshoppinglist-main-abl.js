...
const Errors = require("../api/errors/pfshoppinglist-main-error.js");
...
const logger = LoggerFactory.get("PFShoppingListMainAbl");

class PFShoppingListMainAbl {
  constructor() {
  ...
  async init(uri, dtoIn, session) {
    ...
    // HDS 2
    const schemas = ["pFShoppingListMain"];
    ...
    if (dtoIn.uuBtLocationUri) {
      ...
      const createAwscDtoIn = {
        name: "PFPFShoppingList",
        typeCode: "PF-PFShoppingList-main",
        location: location,
...
module.exports = new PFShoppingListMainAbl();
